...
```{r get_data}
# df_site gets the historial data - if you use df_time you get data from the crop season
...
df_daymet %>% 
  ...
  ggplot(aes(x = Year)) +
  geom_jitter(aes(y = Tmax), alpha = 0.01, color = "red") +
  geom_jitter(aes(y = Tmin), alpha = 0.01, color = "blue") +
...
```{r}
# data_site gets the historial data - if you use data_time you get data from the crop season
#dpp default = 0 - dpp = days prior start date

df_nasapower <- weather_nasapower(input = df_site, dpp = 0)
```
...
There is also codes to get historical summary intervals in the "code_agromet_R.rmd". I don't think we need it. 
